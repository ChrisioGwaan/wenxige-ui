...
export default function ShipmentsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      const { data: rows } = await supabase
        .from('order_shipment')
        .select(
          'id, order_id, carrier_code, tracking_number, tracking_url, shipment_status, estimated_delivery, shipped_at, delivered_at',
        )
      ...
      setShipments(
        rows.map((r) => ({ ...r, order_number: orderMap.get(r.order_id) ?? r.order_id })),
      )
      setLoading(false)
    }
    load()
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter(
        (s) =>
          s.order_number?.toLowerCase().includes(q) || s.tracking_number?.toLowerCase().includes(q),
      )
    }
  ...
  const carriers = useMemo(() => {
    ...
    shipments.forEach((s) => {
      if (s.carrier_code) seen.add(s.carrier_code)
    })
  ...
  const columns: ColumnsType<Shipment> = [
    ...
    },
    {
      ...
      width: 100,
      render: () => (
        <Button type="link" size="small" style={{ padding: 0, color: '#9AB17A' }}>
          {t.shipments.update}
        </Button>
      ),
    },
    ...
    <div>
      ...
      <Card style={{ borderRadius: 12 }}>
        <Row gutter={[12, 12]} style={{ marginBottom: 16 }}>
          ...
          <Col xs={12} md={6}>
            <Select
              ...
              allowClear
              value={filterCarrier}
              onChange={(v) => setFilterCarrier(v ?? null)}
            >
              {carriers.map((c) => (
          ...
          <Col xs={12} md={6}>
            <Select
              placeholder={t.shipments.allStatus}
              style={{ width: '100%' }}
              allowClear
              value={filterStatus}
        ...
        <Table
          ...
          pagination={{
            pageSize: 10,
            showTotal: (total) => `${total} ${t.shipments.shipmentsCount}`,
          }}
          size="middle"
